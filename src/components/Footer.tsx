...
const Footer = memo(() => {
  ...
  return (
    <footer style={{ backgroundColor: theme.colors.primary }} className="text-[var(--footer-text)]">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
          ...
          <div className="col-span-1 flex flex-col items-center md:col-span-2 md:items-start">
            ...
            <p className="mb-3 max-w-md text-sm text-[var(--footer-text-soft)] md:text-base">
            ...
            <p className="font-semibold italic text-teal-200">{config["site.tagline"] || "Africa Inspired!"}</p>
          </div>

          <div>
            <h4 className="mb-4 text-base font-semibold text-[var(--footer-text)] md:text-lg">Quick Links</h4>
            <ul className="space-y-2 text-sm text-[var(--footer-text-soft)] md:text-base">
          ...
          <div>
            ...
            <div className="space-y-3 text-sm text-[var(--footer-text-soft)] md:text-base">
              ...
              {config["contact.phone"] && (() => {
                const raw = String(config["contact.phone"] || "");
                const digits = raw.replace(/[^0-9+]/g, "");
                const waNumber = digits.replace(/\+/g, "");
                const waHref = `https://wa.me/${waNumber}`;
                return (
                  <div className="flex items-center justify-center gap-2 md:justify-start">
                    <a href={waHref} target="_blank" rel="noreferrer" aria-label={`Chat on WhatsApp ${raw}`} className="inline-flex items-center gap-2 text-teal-200 hover:underline">
                      <span className="footer-whatsapp-icon whatsapp-float__icon" style={{ width: 22, height: 22 }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" role="presentation" focusable="false" aria-hidden>
          ...
          <p className="text-sm font-medium text-[var(--footer-text-soft)] md:text-base">
